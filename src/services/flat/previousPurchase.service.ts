...
@Injectable()
export class PreviousPurchaseService {
  constructor(
  ...
  async getpreviousPurchaseList(pagingDto: PagingDto) {
    const manager = getManager(Environment.postgres.flatdatabase)
    const listData = await manager
      .createQueryBuilder()
      .select("*")
      .from(FlatTableList.previousPurchase, FlatTableList.previousPurchase)
      .getRawMany();
    return listData;
  }
